...
export const AuthScreen: React.FC<Props> = ({ onLoginSuccess }) => {
    ...
    const handleAccessRequest = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!email.includes('@')) return setError("IDENTIFICACIÓN INVÁLIDA");
        ...
        if (success) {
            ...
            if (profile) {
                ...
                setError("ERROR DE ENLACE DE DATOS");
                setStatus('idle');
            }
        } else {
    ...
    return (
        <div className="min-h-screen bg-[#050507] flex flex-col items-center justify-between p-6 text-white font-sans relative overflow-hidden">
            ...
            <div className="absolute inset-0 pointer-events-none z-0">
                ...
                <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[900px] h-[900px] blur-[150px] rounded-full transition-all duration-1000 ${isAdminMode ? 'bg-red-600/20' : 'bg-blue-600/5'}`} />
                <div className="scanline-effect"></div>
            </div>

            {/* TOP SECTION: BRANDING & LOGO */}
            <div className="relative z-10 pt-12 flex flex-col items-center space-y-6 animate-in fade-in slide-in-from-top-10 duration-1000">
                <div className="relative group">
                    ...
                    <div className={`relative w-32 h-32 md:w-40 md:h-40 bg-black border-2 rounded-full flex items-center justify-center overflow-hidden shadow-2xl transition-colors duration-500 ${isAdminMode ? 'border-red-600' : 'border-white/10'}`}>
                        {/* REEMPLAZAR CON TU LOGO SI TIENES LA URL */}
                        <div className="flex flex-col items-center">
                            <span className={`text-6xl md:text-7xl font-black italic tracking-tighter leading-none transition-colors ${isAdminMode ? 'text-red-600' : 'text-white'}`}>K</span>
                            <div className={`h-1.5 w-10 mt-1 ${isAdminMode ? 'bg-red-600' : 'bg-white/20'}`} />
                        </div>
                ...
                <div className="text-center space-y-1">
                    <h1 className="text-6xl md:text-8xl font-black italic uppercase tracking-tighter leading-[0.85]">
                        KINETIX<br/>
                        <span className={isAdminMode ? 'text-red-600 drop-shadow-[0_0_25px_rgba(220,38,38,0.5)]' : 'text-white'}>ELITE</span>
                    </h1>
                    <div className="flex flex-col items-center mt-4">
                        <div className={`h-[1px] w-24 mb-2 ${isAdminMode ? 'bg-red-600' : 'bg-white/20'}`} />
            ...
            <div className="w-full max-w-sm relative z-10 space-y-8 mb-12">
                <div className="text-center">
                    <div className={`inline-block px-4 py-1.5 rounded-sm border transition-all duration-500 ${isAdminMode ? 'bg-red-600/10 border-red-500/40' : 'bg-white/5 border-white/10'}`}>
                        <p className={`text-[9px] font-black uppercase tracking-[0.6em] ${isAdminMode ? 'text-red-500 animate-pulse' : 'text-white/30'}`}>
                            {isAdminMode ? 'SISTEMA ALPHA: IDENTIFICADO' : 'Terminal de Acceso Milspec'}
                        </p>
                    </div>
                </div>
                ...
                <form onSubmit={handleAccessRequest} className="space-y-4">
                    <div className="space-y-3">
                        <div className="group relative">
                            <input 
                                type="email" 
                                placeholder="IDENTIFICADOR (CORREO)" 
                                className={`w-full bg-black/60 backdrop-blur-xl border p-5 rounded-none font-black text-[12px] text-center uppercase tracking-[0.3em] outline-none transition-all placeholder-white/10 ${isAdminMode ? 'border-red-600/50 focus:border-red-500' : 'border-white/10 focus:border-white/40 focus:bg-white/10'}`}
                                value={email}
                        ...
                        {isAdminMode && (
                            <div className="relative animate-in slide-in-from-top-4 duration-500">
                                <input 
                                    ...
                                    className="w-full bg-red-950/20 border border-red-600/50 p-5 rounded-none font-black text-[12px] text-center uppercase tracking-[0.6em] outline-none focus:border-red-500 transition-all placeholder-red-900/40 text-red-600"
                                    value={password}
                                    onChange={e => setPassword(e.target.value)}
                                    disabled={status !== 'idle'}
                                />
                                <div className="absolute top-0 right-4 h-full flex items-center">
                                    <span className="text-[8px] font-black text-red-600 opacity-40">MASTER_AUTH</span>
                                </div>
                            </div>
                        )}
                    </div>
                    ...
                    {error && (
                        <div className="p-4 bg-red-950/40 border border-red-600/40 rounded-none animate-in zoom-in-95">
                            <p className="text-[10px] text-red-500 font-black uppercase text-center tracking-[0.2em]">{error}</p>
                        </div>
                    )}
                    ...
                    >
                        {status === 'loading' ? 'VERIFICANDO...' : status === 'sent' ? 'ACCESO OK' : isAdminMode ? 'DESBLOQUEAR MANDO' : 'SOLICITAR ACCESO'}
            ...
            <div className="relative z-10 w-full max-w-lg pb-10 flex flex-col items-center space-y-8">
                <div className="flex gap-10 items-center">
                    ...
                    <a href="https://facebook.com/kinetix.funcionalzone" target="_blank" rel="noreferrer" className="group flex flex-col items-center gap-2">
                        ...
                        <span className="text-[8px] font-black uppercase text-white/20 tracking-widest group-hover:text-white transition-colors">Facebook</span>
                    </a>
                    {/* Instagram */}
                    <a href="https://instagram.com/kinetix.funcionalzone" target="_blank" rel="noreferrer" className="group flex flex-col items-center gap-2">
                        ...
                        <span className="text-[8px] font-black uppercase text-white/20 tracking-widest group-hover:text-white transition-colors">Instagram</span>
                    </a>
                    {/* WhatsApp */}
                    <a href="https://wa.me/525627303189" target="_blank" rel="noreferrer" className="group flex flex-col items-center gap-2">
                        ...
                        <span className="text-[8px] font-black uppercase text-white/20 tracking-widest group-hover:text-white transition-colors">WhatsApp</span>
                    </a>
                </div>
                ...
                <div className="flex flex-col items-center gap-4 pt-8 w-full border-t border-white/5">
                    <div className="flex items-center gap-6">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-[9px] font-black text-red-600/60 uppercase tracking-widest">Master Protocol Online</span>
                        </div>
                        <div className="h-3 w-[1px] bg-white/10" />
                        <span className="text-[9px] font-black text-white/10 uppercase tracking-widest">Ver: 140.0_OBSIDIAN</span>
                    </div>
                </div>
            </div>
