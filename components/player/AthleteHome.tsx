...
const TechnicalChatModal: React.FC<{ onClose: () => void }> = ({ onClose }) => {
    const [messages, setMessages] = useState<{role: 'user' | 'ai', text: string}[]>([
        { role: 'ai', text: 'Kinetix Ops Online. Soy experto en biomecánica. ¿Dudas con la técnica de algún movimiento?' }
    ]);
            ...
            <div className="bg-[#0F0F11] w-full md:max-w-md h-[85vh] md:h-[600px] md:rounded-[32px] rounded-t-[32px] border border-blue-500/20 shadow-[0_0_50px_rgba(59,130,246,0.15)] flex flex-col overflow-hidden relative">
                <div className="p-5 border-b border-white/5 flex justify-between items-center bg-[#0F0F11]">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-[8px] font-black uppercase text-blue-400/50 tracking-widest">IA BIOMECÁNICA ACTIVA</p>
                        </div>
                    </div>
                ...
                <div ref={scrollRef} className="flex-1 overflow-y-auto p-5 space-y-4 custom-scrollbar bg-gradient-to-b from-[#0F0F11] to-black">
                    {messages.map((m, i) => (
                        <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                            <div className={`max-w-[85%] p-4 rounded-2xl text-xs font-medium leading-relaxed ${m.role === 'user' ? 'bg-blue-900/20 text-white border border-blue-900/40 rounded-br-none' : 'bg-[#1A1A1D] text-white/80 border border-white/5 rounded-bl-none'}`}>
                                {m.text}
                            </div>
                        </div>
                    ))}
                ...
                <div className="p-4 bg-[#0F0F11] border-t border-white/5">
                    <div className="flex gap-2">
                        <input type="text" className="flex-1 bg-black border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:border-blue-600 outline-none placeholder-white/20" placeholder="Consultar técnica..." value={input} onChange={e => setInput(e.target.value)} />
      ...
      <main className="max-w-md mx-auto pt-8 px-6 pb-32">
         ...
         {activeTab === 'train' && (
             <div className="space-y-8 animate-in slide-in-from-left-4">
                 ...
                 {/* RECONOCIMIENTO DE MAÑANA (INTEL) */}
                     ...
                     <button onClick={() => setShowChat(true)} className="w-full bg-[#151518] border border-blue-500/10 hover:border-blue-500/40 p-5 rounded-[28px] flex items-center justify-between group transition-all">
                        <div className="flex items-center gap-4">
                            ...
                            <div className="text-left">
                                <p className="text-[10px] font-black uppercase tracking-widest text-white">Kinetix Ops Intelligence</p>
                                <p className="text-[8px] text-blue-400/50 uppercase font-bold">Terminal de Asistencia</p>
                            </div>
                        </div>
